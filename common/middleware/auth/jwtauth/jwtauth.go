...
const JWTLoginPrefix = "login:jwt"
...
func (mw *GinJWTMiddleware) GetClaimsFromJWT(c *gin.Context) (MapClaims, error) {
	...
	saveTokenStr, _ := sdk.Runtime.GetCacheAdapter().Get(JWTLoginPrefix, claims["identity"].(string))
	//单点登录
...
func (mw *GinJWTMiddleware) LoginHandler(c *gin.Context) {
	...
	err = sdk.Runtime.GetCacheAdapter().Set(JWTLoginPrefix, claims["identity"].(string), tokenString, -1)
	if err != nil {
		mw.unauthorized(c, http.StatusInternalServerError, mw.HTTPStatusMessageFunc(ErrMissingAuthenticatorFunc, c))
		return
	}
