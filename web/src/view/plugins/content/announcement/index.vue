...
<template>
  <div>
    <div class="gva-search-box">
      <el-form ref="searchForm" :model="queryParams" :inline="true" label-position="left">
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="queryParams.status" placeholder="公告管理状态" clearable size="small">
            <el-option
    ...
    <el-table v-loading="loading" stripe border :data="announcementList">
      ...
      <el-table-column min-width="100" label="标题" align="center" prop="title" :show-overflow-tooltip="true" />
      <el-table-column min-width="100" label="内容" align="center" prop="content" :show-overflow-tooltip="true" />
      <el-table-column min-width="100" label="阅读次数" align="center" prop="num" :show-overflow-tooltip="true" />
      ...
      <el-table-column min-width="150" label="更新时间" align="center" prop="updatedAt" :show-overflow-tooltip="true">
        <template #default="scope">
    ...
    <el-dialog v-model="open" :close-on-click-modal="false" :title="title" width="500px" append-to-body destroy-on-close>
      <el-form ref="inputform" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="阅读次数" prop="num">
          <el-input v-model="form.num" type="number" placeholder="阅读次数" />
        </el-form-item>
...
import { exportAnnouncement, addAnnouncement, delAnnouncement, getAnnouncement, listAnnouncement, updateAnnouncement } from '@/api/plugins/content/announcement'
...
const getList = () => {
  loading.value = true
  listAnnouncement(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => {
...
const submitForm = () => {
  inputform.value.validate(valid => {
    if (valid) {
      form.value.num = Number(form.value.num)
...
const handleDelete = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + ids + '"的数据项?', '警告', {
    ...
    return delAnnouncement({ 'ids': ids })
  }).then((response) => {
    if (response.code === 200) {
      open.value = false
      ElMessage({
...
const handleExport = () => {
  ElMessageBox.confirm('是否确认导出所选数据？', '提示', {
    ...
    exportAnnouncement(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => {
      resolveBlob(response, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', '公告管理')
    })
  }).catch(() => {
  })
}
