...
<template>
  <div>
    ...
    <el-table v-loading="loading" stripe border :data="categoryList">
...
import { exportCategory, addCategory, delCategory, getCategory, listCategory, updateCategory } from '@/api/plugins/content/category'
...
const handleExport = () => {
  ElMessageBox.confirm('是否确认导出所选数据？', '提示', {
    ...
    exportCategory(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => {
      resolveBlob(response, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', '文章分类管理')
    })
  }).catch(() => {
  })
}
